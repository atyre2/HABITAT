...
int init_rep(Parameters *Pm, LParameters *LPm){
  ...
  Form1->StatusBar1->SimpleText = "Initializing Dispersal Queue...";
  Dispersers = InitQueue();

  Form1->StatusBar1->SimpleText = "Initializing Main List...";
  Population = InitList();

  Form1->StatusBar1->SimpleText = "Initializing Population...";
  InitPop(Population,Pm);         /* initialize the population */

  Form1->StatusBar1->SimpleText = "Initializing Landscape...";
  if (((Pm->Run == 1)&&(Pm->Rep == 0)) || (Pm->NewLand))
  {
      AdaBlock = InitLandScape(Pm,LPm);
      Form1->DrawLandscape(AdaBlock);
  }

  Form1->StatusBar1->SimpleText = "Building Transform Tables, Run #" + IntToStr(Pm->Run);
  ...
  Form1->DrawPopulation(AdaBlock,Population);
...
int step_rep(Parameters *Pm, LParameters *LPm){
  ...
  Form1->StatusBar1->SimpleText = "Year: " + IntToStr(Pm->Year) +
...
void load_parameters(Parameters *Pm, LParameters *LPm)
{
  ...
  int NumTries;
  int iExtinct;
  float FecundityMultiplier = 1.0;
  int RandomSensitivity = 0;
  char randvar[20];
  FILE *RandomFile;
